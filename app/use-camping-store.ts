...
function prepareCampingData(value: unknown): CampingData {
  ...
  if (incoming.routes.some((route) => !siteIds.has(route.startSiteId) || !siteIds.has(route.endSiteId) || (route.tripId && !tripIds.has(route.tripId))) || incoming.visits.some((visit) => !siteIds.has(visit.siteId) || (visit.tripId && !tripIds.has(visit.tripId)))) throw new Error('Backupfilen har ruter eller besøg med manglende relationer.');

  const clean: CampingData = {
    schemaVersion: 1,
    trips: incoming.trips.map((trip) => ({ id: trip.id, title: trip.title, startDate: trip.startDate, endDate: trip.endDate, status: trip.status, region: trip.region, summary: trip.summary, destinationIds: [...trip.destinationIds], participantIds: [...trip.participantIds], petIds: [...trip.petIds], coverTone: trip.coverTone })),
    sites: incoming.sites.map((site) => ({ id: site.id, name: site.name, place: site.place, country: site.country, coordinates: [site.coordinates[0], site.coordinates[1]], status: site.status, rating: site.rating, visits: site.visits, tags: [...site.tags], note: site.note, favorite: site.favorite })),
    experiences: incoming.experiences.map((item) => ({ id: item.id, title: item.title, place: item.place, date: item.date, status: item.status, note: item.note, ...(item.tripId ? { tripId: item.tripId } : {}) })),
    routes: incoming.routes.map((route) => ({ id: route.id, name: route.name, startSiteId: route.startSiteId, endSiteId: route.endSiteId, profile: route.profile, createdAt: route.createdAt, ...(route.tripId ? { tripId: route.tripId } : {}), ...(route.distanceKm !== undefined ? { distanceKm: route.distanceKm } : {}), ...(route.durationMinutes !== undefined ? { durationMinutes: route.durationMinutes } : {}), geometry: route.geometry.map((coordinate) => [coordinate[0], coordinate[1]] as [number, number]) })),

...
const roleDetails = [
  { name: 'Sisi', role: 'Ferie Vagten', image: '/sisi-vagten.png', status: 'Alt ser godt ud', task: 'Kontrollerer relationer og mangler', tone: 'green', description: 'Holder øje med din aktive ferie, finder manglende oplysninger og samler nye hændelser det rigtige sted.' },
  { name: 'Sisi', role: 'Ferie Guiden', image: '/sisi-guiden.png', status: 'Næste stop er klar', task: 'Giver kontekstuelle forslag', tone: 'sand', description: 'Viser næste gode skridt og hjælper dig direkte videre til den funktion, du skal bruge.' },
  { name: 'Misser', role: 'Ferie Grafikeren', image: '/misser-grafikeren.png', status: 'Albummet vokser', task: 'Organiserer minder og kapitler', tone: 'navy', description: 'Samler billeder, steder, ruter og noter til et overskueligt feriealbum, som du selv styrer.' },
  { name: 'Misser', role: 'Ferie Meteorologen', image: '/misser-meteorologen.png', status: 'Vejret er opdateret', task: 'Følger prognosen for dit stop', tone: 'teal', description: 'Kobler vejret til din aktuelle placering og viser senest kendte data, hvis forbindelsen forsvinder.' },
];

type ModalId = 'trip' | 'site' | 'visit' | 'note' | 'experience' | 'person' | 'role' | null;
...
export default function CampingApp() {
  ...
  const guideTarget: ViewId = !activeTrip ? 'trips' : !activeSite ? 'sites' : view === 'dashboard' ? 'map' : view === 'map' ? 'experiences' : 'dashboard';
  const roleStates = roleDetails.map((role, index) => ({
    ...role,
    status: index === 0 ? (guardChecks.length ? `${guardChecks.length} ${guardChecks.length === 1 ? 'punkt kræver blik' : 'punkter kræver blik'}` : 'Alt ser godt ud') : index === 1 ? (!activeTrip ? 'Opret din første ferie' : !activeSite ? 'Vælg næste destination' : 'Næste skridt er klar') : index === 2 ? (data.media.length ? `${data.media.length} minder samlet` : 'Klar til første minde') : activeSite ? `Følger vejret i ${activeSite.place}` : 'Venter på en destination',
    task: index === 0 ? (guardChecks.some((item) => item.severity === 'problem') ? 'Problem fundet' : guardChecks.length ? 'Opmærksomhed' : 'Kontrol gennemført') : role.task,
  }));
  ...
  function removeRoute(id: string) {
    ...
    if (!route || !confirm(`Slet etapeudkastet “${route.name}”?`)) return;
  ...
  async function removeMedia(id: string) {
    ...
    if (!item || !confirm(`Slet billedet “${item.name}” fra denne enhed?`)) return;
    try {
  ...
  function removeTrip(id: string) {
    ...
    if (!trip || !confirm(`Slet ferien “${trip.title}”? Steder, noter og billeder beholdes, men ferietilknytningen fjernes.`)) return;
    mutate((draft) => {
  ...
  function removeSite(id: string) {
    ...
    if (!site || !confirm(`Slet campingpladsen “${site.name}”?`)) return;
    mutate((draft) => {
  ...
  function removeNote(id: string) {
    ...
    if (!note || !confirm(`Slet noten “${note.title}”?`)) return;
  ...
  function removeExperience(id: string) {
    ...
    if (!item || !confirm(`Slet oplevelsen “${item.title}”?`)) return;
  ...
  function removePerson(id: string) {
    ...
    if (!person || !confirm(`Slet profilen “${person.name}”?`)) return;
    mutate((draft) => {
  ...
  async function importBackup(event: ChangeEvent<HTMLInputElement>) {
    ...
    const hasCurrentData = data.trips.length + data.sites.length + data.experiences.length + data.routes.length + data.visits.length + data.notes.length + data.media.length + data.people.length + data.events.length > 0 || data.settings.mapStyle !== 'liberty' || data.settings.automationMode !== 'automatic' || data.settings.weatherEnabled || data.settings.reducedMotion;
  ...
  return (
    <main className={`app-shell ${data.settings.reducedMotion ? 'reduce-motion' : ''}`}>
      ...
      <section className="workspace">
        ...
        <div className="content">
          {view === 'dashboard' && <>
            ...
            <section className="section-block"><div className="section-heading"><div><p className="eyebrow">Ferie Administrationen</p><h2>Dit rejsehold er på arbejde</h2></div><span className="section-count">4 roller · én fælles ferie</span></div><div className="role-grid">{roleStates.map((role, index) => <article className={`role-card ${role.tone} ${index === 0 && guardChecks.some((item) => item.severity === 'problem') ? 'problem' : ''}`} key={role.role}><Image src={role.image} width={124} height={124} alt={`${role.name} som ${role.role}`} /><div><p>{role.name}</p><h3>{role.role}</h3><strong><span />{role.status}</strong><small>{role.task}</small></div><button onClick={() => { setSelectedRole(index); setModal('role'); }} type="button" aria-label={`Åbn ${role.role}`}>→</button></article>)}</div></section>
          ...
          {view === 'settings' && <section className="page-view">
            <div className="page-heading"><div><p className="eyebrow">Din app</p><h2>Indstillinger</h2><p>Alt her gemmes kun på enheden. API-hemmeligheder indgår aldrig i backup.</p></div></div>
            <div className="settings-grid">
              <article className="settings-card"><h3>Automatisering</h3>
              ...
              <article className="settings-card"><h3>Kort</h3><label className="field-label">Standardkort<select value={data.settings.mapStyle} onChange={(event) => mutate((draft) => { draft.settings.mapStyle = event.target.value as CampingData['settings']['mapStyle']; })}><option value="liberty">Liberty</option><option value="bright">Bright</option><option value="positron">Positron</option><option value="dark">Dark</option><option value="fiord">Fiord</option></select></label><p className="provider-note">Kort: OpenFreeMap/OpenStreetMap. Ruter med specialprofil kræver en serverkonfigureret nøgle, som ikke gemmes eller eksporteres.</p></article>
              <article className="settings-card data-card"><h3>Data & backup</h3><p>Eksporten indeholder relationer og billedfiler, men aldrig hemmelig tjenestekonfiguration. Samme sikkerhedsgrænse på 250 MB gælder for eksport og import.</p><div className="stack-actions"><button type="button" onClick={() => void exportBackup()}>Eksportér komplet JSON-backup</button><button type="button" onClick={() => importInputRef.current?.click()}>Importér backup</button><button type="button" onClick={async () => { if (confirm('Gendan eksempeldata og fjern egne mediefiler fra appens lokale lager?')) { await store.resetToSample(); setToast('Eksempeldata er gendannet.'); } }}>Gendan eksempeldata</button><button className="danger" type="button" onClick={async () => { if (confirm('Slet alle Vores Camping-data på denne enhed? Handlingen kan ikke fortrydes uden en backup.')) { await store.clearAll(); setToast('Alle lokale appdata og vejrpositioner er slettet.'); } }}>Slet alle lokale data</button></div></article>
            </div>
          </section>}
      ...
      {modal && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setModal(null); }}><section ref={modalRef} className="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title"><button className="modal-close" type="button" onClick={() => setModal(null)} aria-label="Luk">×</button>
        ...
        {modal === 'role' && <div className="role-modal"><Image src={roleDetails[selectedRole].image} width={210} height={210} alt="" /><div><p className="eyebrow">{roleDetails[selectedRole].name}</p><h2 id="modal-title">{roleDetails[selectedRole].role}</h2><p>{roleDetails[selectedRole].description}</p>{selectedRole === 0 ? <div className="role-insights">{guardChecks.length ? guardChecks.map((check, index) => <button type="button" key={`${check.title}-${index}`} onClick={() => { setModal(null); changeView(check.target); }}><span className={check.severity}>{check.severity === 'problem' ? '!' : '•'}</span><div><strong>{check.title}</strong><small>Gå direkte til rettelsen</small></div>→</button>) : <div className="all-clear"><span>✓</span><strong>Alle relationer ser sunde ud</strong></div>}</div> : <ul><li>Arbejder kun med dine appdata</li><li>Viser altid hvad der er foreslået</li><li>Du vælger selv, om forslaget skal bruges</li></ul>}<button className="primary-button" type="button" onClick={() => { const target = selectedRole === 0 ? guardChecks[0]?.target ?? 'dashboard' : selectedRole === 1 ? guideTarget : selectedRole === 2 ? 'album' : activeSite ? 'dashboard' : 'sites'; setModal(null); changeView(target); }}>{selectedRole === 0 ? (guardChecks.length ? 'Åbn første punkt' : 'Tilbage til overblik') : selectedRole === 1 ? 'Følg guidens forslag' : selectedRole === 2 ? 'Åbn feriealbum' : activeSite ? 'Se vejret' : 'Vælg destination'}</button></div></div>}
      </section></div>}
    </main>
  );
}
